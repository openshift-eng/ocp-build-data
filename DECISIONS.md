# DECISIONS.md — ACM 2.17 ocp-build-data Branch

Product decisions and rationale for the `acm-2.17` branch configuration.
Created as part of JIRA ticket [HYPBLD-872](https://redhat.atlassian.net/browse/HYPBLD-872). Carried forward from `acm-2.16`; update entries below as 2.17-specific decisions are confirmed.

## Product Identity

- **Decision**: `product: rhacm2`, `name: acm-2.17`, `csv_namespace: open-cluster-management`
...
- **Rationale**: ACM 2.17 continues to align with OCP 4.21 infrastructure (unchanged from 2.16). The distgit branch `rhaos-4.21-rhel-9` is shared Brew infrastructure tied to OCP releases. All layered products use OCP version numbers for MAJOR.MINOR. **Note**: `branch-setup` initially set `MAJOR: 2` / `MINOR: 17` from the product version; corrected back to `4` / `21` manually.
- **Source**: `acm-redhat-operators-config.yaml`, ART OLM Bundle docs.

## OCP Target Versions

- **Decision**: `OCP_TARGET_VERSIONS: ["4.18", "4.19", "4.20", "4.21", "4.22"]`
- **Rationale**: Carried forward from ACM 2.16 catalog targets; confirm against `acm-mce-operator-catalogs/config/acm-redhat-operators-config.yaml` for 2.17 before GA.
