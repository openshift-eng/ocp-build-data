...
ENV container=oci \
    MANPATH=/opt/rh/go-toolset-1.14/root/usr/share/man: \
    X_SCLS=go-toolset-1.14  \
    GOFLAGS='-mod=vendor' \
    LD_LIBRARY_PATH=/opt/rh/go-toolset-1.14/root/usr/lib64 \
    PATH=/opt/rh/go-toolset-1.14/root/usr/bin:/opt/rh/go-toolset-1.14/root/usr/sbin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin \
    GOPATH=${GOPATH:-/go} \
    PKG_CONFIG_PATH=/opt/rh/go-toolset-1.14/root/usr/lib64/pkgconfig

RUN mkdir -p /go/src/

RUN yum install -y --setopt=skip_missing_names_on_install=False \
    bc file findutils gpgme git hostname lsof make socat tar tree util-linux wget which zip \
    'go-toolset-1.14*' goversioninfo openssl openssl-devel systemd-devel gpgme-devel libassuan-devel
RUN yum clean all
